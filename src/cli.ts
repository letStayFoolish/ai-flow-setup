...
import type { SkillInstallMode } from "./lib/prompts.js";
import { installEntries } from "./lib/install.js";
import { installSkills } from "./lib/skillInstall.js";

const SKILL_INSTALL_MODES: SkillInstallMode[] = ["copy-global", "symlink-global", "project-local"];

function parseMode(raw: string): SkillInstallMode {
  if (!SKILL_INSTALL_MODES.includes(raw as SkillInstallMode)) {
    clack.log.error(`invalid --mode "${raw}" — must be one of: ${SKILL_INSTALL_MODES.join(", ")}`);
    process.exit(1);
  }
  return raw as SkillInstallMode;
}

async function resolveSelection(catalog: CatalogEntry[], names: string[]): Promise<CatalogEntry[]> {
  if (names.length === 0) {
    clack.note(
      [
  ...
  for (const name of unmatched) clack.log.error(`no match for "${name}"`);
  ...
  if (unmatched.length > 0 || ambiguous.length > 0) process.exit(1);
  return matched;
}
...
program
  ...
  .option(
    "--mode <mode>",
    `global skill install mode (${SKILL_INSTALL_MODES.join("|")})`,
    "copy-global",
  )
  .action(async (names: string[], opts: { mode: string }) => {
